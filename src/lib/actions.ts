...
export async function createMeetingPrepAction(
  ...
  try {
    const shareToken = nanoid(16);
    const shareExpiryDate = addDays(new Date(), shareExpiryDays !== undefined ? shareExpiryDays : 7); // Default 7 days

    const newMeetingPrep = await dbAddMeetingPrep({
      ...restPayload,
      friendGroupId, // Add friendGroupId to the object being saved
      creatorId: creator.id,
      shareToken,
      shareExpiryDate,
    });
...
export async function getMeetingPrepsAction(currentUserId: string | null) {
  ...
  try {
    ...
    if (user.role === 'admin') {
      ...
      // Fetch meeting preps where the user is the creator or a participant
      meetingPreps = await dbGetMeetingPrepsByUser(user.id, user.friendGroupIds || []);
    }

    // Fetch friend details for each meeting prep's participants
...
export async function getMeetingPrepByIdAction(meetingPrepId: string, currentUserId?: string | null) {
  try {
    ...
    // If currentUserId is provided, check permissions
    if (currentUserId) {
      const permissionCheck = await _ensureUserPermission(currentUserId, {
      ...
      const isAdmin = user.role === 'admin';
      // Check if any of the user's friends are participants in this meeting prep
      const userFriends = await dbGetFriendsByUserFriendGroupIds(user.friendGroupIds || []);
      const isParticipant = userFriends.some(userFriend => meetingPrep.participantFriendIds?.includes(userFriend.id));
    ...
    // Fetch participant friend details
...
export async function toggleMeetingPrepShareAction(meetingPrepId: string, currentUserId: string, enable: boolean, expiryDays: number = 7) {
  const permissionCheck = await _ensureUserPermission(currentUserId, { entityName: '모임 준비 공유 설정 변경' });
  if (!permissionCheck.success) {
    return { success: false, error: permissionCheck.error };
  }
  const user = permissionCheck.user!;
...
export async function submitParticipantAvailabilityAction(
  ...
  try {
    ...
    if (currentUserId) {
      const userResult = await _ensureUserPermission(currentUserId);
    ...
    if (existingAvailability) {
      ...
      // New submission: no existing record, so just add it.
...
export async function getParticipantAvailabilityAction(meetingPrepId: string, selectedFriendId: string, currentUserId?: string | null) {
  try {
    ...
    if (availability) {
      // Reconstruct availableDates and unavailableDates
      ...
      meetingPrep.selectedMonths.forEach(monthStr => {
        ...
        for (let i = 1; i <= daysInMonth; i++) {
          ...
          if (date >= today || isSameDay(date, today)) { // Only include today and future dates
...
export async function getAllParticipantAvailabilitiesAction(meetingPrepId: string, currentUserId?: string | null) {
  try {
    ...
    // Reconstruct availableDates and unavailableDates for each availability
    const allDatesInPrep: string[] = [];
    const today = new Date();
    meetingPrep.selectedMonths.forEach(monthStr => {
      const [year, month] = monthStr.split('-').map(Number);
